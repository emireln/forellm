#!/usr/bin/env bash
# Automated model database update script for ForeLLM
# This script:
...
echo -e "${BLUE}━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━${NC}"
echo -e "${BLUE}  ForeLLM Model Database Update${NC}"
echo -e "${BLUE}━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━${NC}"
echo
...
if command -v cargo &> /dev/null; then
    ...
    echo -e "${BLUE}🔨 Rebuilding ForeLLM with updated model data...${NC}"
    cargo build --release
    ...
    if [ $? -eq 0 ]; then
        ...
        if [ -f "$PROJECT_ROOT/target/release/forellm" ]; then
            BINARY_SIZE=$(ls -lh "$PROJECT_ROOT/target/release/forellm" | awk '{print $5}')
            echo -e "${GREEN}📦 Binary location:${NC} target/release/forellm (${BINARY_SIZE})"
        fi
    else
...
echo "  • Run './target/release/forellm' to test the updated binary"
