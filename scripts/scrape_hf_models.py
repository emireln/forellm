...
def _auth_headers() -> dict[str, str]:
    ...
    headers = {"User-Agent": "forellm-scraper/1.0"}
    if _hf_token:
...
def main():
    parser = argparse.ArgumentParser(
        description="Scrape LLM model metadata from HuggingFace for ForeLLM."
    )
    parser.add_argument(
    ...
    # Write to both locations: repo root (for reference) and forellm-core (compiled into binary)
    output_paths = ["data/hf_models.json", "forellm-core/data/hf_models.json"]
