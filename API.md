# ForeLLM REST API Guide

This document is for agent/client builders integrating with `forellm serve`.

## Purpose

`forellm serve` exposes node-local model fit analysis (same core data used by TUI/CLI) over HTTP.
...
```sh
forellm --memory 24G --max-context 8192 serve --host 0.0.0.0 --port 8787
```
