use forellm_core::fit::{FitLevel, ModelFit, SortColumn, backend_compatible};
use forellm_core::hardware::SystemSpecs;
use forellm_core::models::{Capability, ModelDatabase, UseCase};
use forellm_core::plan::{PlanEstimate, PlanRequest, estimate_model_plan};
...
impl App {
        ...
        self.filtered_fits = self
            ...
            .filter(|(_, fit)| {
                ...
                let is_apple_silicon = self.specs.backend
                    == forellm_core::hardware::GpuBackend::Metal
    ...
    fn re_sort(&mut self) {
        ...
        self.all_fits = forellm_core::fit::rank_models_by_fit_opts_col(
            fits,
    ...
    pub fn start_download(&mut self) {
        ...
        let use_mlx = fit.runtime == forellm_core::fit::InferenceRuntime::Mlx && self.mlx_available;
