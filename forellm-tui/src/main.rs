...
use forellm_core::fit::{ModelFit, SortColumn, backend_compatible};
use forellm_core::hardware::SystemSpecs;
use forellm_core::models::ModelDatabase;
use forellm_core::plan::{PlanRequest, estimate_model_plan, resolve_model_selector};
...
AGENT USAGE:
  No --json support for this command. Use 'forellm list --json' and filter
  client-side, or use 'forellm info <model> --json' for a specific model.")]
    Search {
...
AGENT USAGE:
  forellm info \"llama-3.1-8b\" --json
...
PRECONDITIONS:
  Model must exist in the embedded database (use 'forellm search' to verify).
...
SIDE EFFECTS:
  ...
  (~/.cache/forellm/models/ or platform equivalent).
...
AGENT USAGE:
  No --json support. Parse the tabular stdout output, or use the ForeLLM
  REST API ('forellm serve') for programmatic access.")]
    HfSearch {
...
PRECONDITIONS:
  ...
  A GGUF model file must exist locally (use 'forellm download' first).
...
AGENT USAGE:
  ...
  Then interact via the OpenAI-compatible API at http://localhost:8080.")]
    ...
    /// Start ForeLLM REST API server for cluster/node scheduling workflows
    #[command(long_about = "\
Start ForeLLM REST API server for cluster/node scheduling workflows.

Exposes ForeLLM's hardware detection and model fitting as a REST API. Useful
...
fn fit_matches_filter(fit: &ModelFit, filter: FitArg) -> bool {
    match filter {
        FitArg::All => true,
        FitArg::Perfect => fit.fit_level == forellm_core::fit::FitLevel::Perfect,
        FitArg::Good => fit.fit_level == forellm_core::fit::FitLevel::Good,
        FitArg::Marginal => fit.fit_level == forellm_core::fit::FitLevel::Marginal,
        FitArg::Tight => fit.fit_level == forellm_core::fit::FitLevel::TooTight,
        FitArg::Runnable => fit.fit_level != forellm_core::fit::FitLevel::TooTight,
    }
}
...
fn run_recommend(
    ...
    let min_level = match min_fit.to_lowercase().as_str() {
        "perfect" => forellm_core::fit::FitLevel::Perfect,
        "good" => forellm_core::fit::FitLevel::Good,
        "marginal" => forellm_core::fit::FitLevel::Marginal,
        _ => forellm_core::fit::FitLevel::Marginal,
    };
    fits.retain(|f| match (min_level, f.fit_level) {
        (forellm_core::fit::FitLevel::Marginal, forellm_core::fit::FitLevel::TooTight) => false,
        (
            forellm_core::fit::FitLevel::Good,
            forellm_core::fit::FitLevel::TooTight | forellm_core::fit::FitLevel::Marginal,
        ) => false,
        (forellm_core::fit::FitLevel::Perfect, forellm_core::fit::FitLevel::Perfect) => true,
        (forellm_core::fit::FitLevel::Perfect, _) => false,
        _ => true,
    });

    // Hide MLX-only models on non-Apple Silicon systems
    let is_apple_silicon =
        specs.backend == forellm_core::hardware::GpuBackend::Metal && specs.unified_memory;
    if !is_apple_silicon {
    ...
    match runtime_filter.to_lowercase().as_str() {
        "mlx" => fits.retain(|f| f.runtime == forellm_core::fit::InferenceRuntime::Mlx),
        "llamacpp" | "llama.cpp" | "llama_cpp" => {
            fits.retain(|f| f.runtime == forellm_core::fit::InferenceRuntime::LlamaCpp)
        }
        "vllm" => fits.retain(|f| f.runtime == forellm_core::fit::InferenceRuntime::Vllm),
    ...
    if let Some(ref uc) = use_case {
        let target = match uc.to_lowercase().as_str() {
            "coding" | "code" => Some(forellm_core::models::UseCase::Coding),
            "reasoning" | "reason" => Some(forellm_core::models::UseCase::Reasoning),
            "chat" => Some(forellm_core::models::UseCase::Chat),
            "multimodal" | "vision" => Some(forellm_core::models::UseCase::Multimodal),
            "embedding" | "embed" => Some(forellm_core::models::UseCase::Embedding),
            "general" => Some(forellm_core::models::UseCase::General),
            _ => None,
        };
            ...
            requested
                ...
                .all(|req| match req.to_lowercase().as_str() {
                    "vision" => f
                        .model
                        .capabilities
                        .contains(&forellm_core::models::Capability::Vision),
                    "tool_use" | "tools" | "tool-use" | "function_calling" => f
                        .model
                        .capabilities
                        .contains(&forellm_core::models::Capability::ToolUse),
                    _ => true,
                })
        });
    }

    fits = forellm_core::fit::rank_models_by_fit(fits);
    fits.truncate(limit);
...
fn run_download(
    ...
    let repo_id = if model.contains('/') {
        model.to_string()
    } else if let Some(repo) = forellm_core::providers::gguf_pull_tag(model) {
        repo
    } else {
        // Search HuggingFace
        println!(
        ...
        if results.is_empty() {
            ...
            eprintln!("Tip: use 'forellm hf-search <query>' to browse available models.");
            std::process::exit(1);
        }
        ...
        Ok(handle) => {
            ...
            loop {
                match handle.receiver.recv() {
                    Ok(forellm_core::providers::PullEvent::Progress { status, percent }) => {
                    ...
                    Ok(forellm_core::providers::PullEvent::Error(e)) => {
                        eprintln!("\n\n✗ Download failed: {}", e);
                        std::process::exit(1);
                    }
                    Err(_) => {
...
fn run_hf_search(query: &str, limit: usize) {
    ...
    println!("\nTo download: forellm download <repository>");
    println!("To list files: forellm download <repository> --list");
}
    ...
    let model_path = if std::path::Path::new(model).exists() {
        ...
        match found {
            ...
            None => {
                ...
                eprintln!("\nUse 'forellm download <model>' to download a model first.");
                std::process::exit(1);
            }
        }
    };
...
mod tests {
    use super::*;
    use forellm_core::fit::{FitLevel, InferenceRuntime, RunMode, ScoreComponents};
    use forellm_core::models::LlmModel;
    ...
    fn mock_fit(name: &str, fit_level: FitLevel) -> ModelFit {
        ModelFit {
            model: LlmModel {
                ...
                gguf_sources: vec![],
                capabilities: vec![],
                format: forellm_core::models::ModelFormat::default(),
            },
            fit_level,
            ...
            use_case: forellm_core::models::UseCase::General,
