...
use forellm_core::fit::{
    FitLevel, InferenceRuntime, ModelFit, SortColumn, backend_compatible,
    rank_models_by_fit_opts_col,
};
use forellm_core::hardware::{GpuBackend, SystemSpecs};
use forellm_core::models::{LlmModel, ModelDatabase, UseCase};
...
pub fn run_serve(
    ...
    let specs = detect_specs(memory_override, ram_override, cores_override);
    ...
    println!("ForeLLM API server listening on http://{}", addr);
    println!("  GET /health");
...
fn run_mode_code(run_mode: forellm_core::fit::RunMode) -> &'static str {
    match run_mode {
        forellm_core::fit::RunMode::Gpu => "gpu",
        forellm_core::fit::RunMode::MoeOffload => "moe_offload",
        forellm_core::fit::RunMode::CpuOffload => "cpu_offload",
        forellm_core::fit::RunMode::CpuOnly => "cpu_only",
    }
}
...
fn detect_specs(
    ...
    if let Some(mem_str) = memory_override {
        if let Some(gb) = forellm_core::hardware::parse_memory_size(mem_str) {
            specs = specs.with_gpu_memory_override(gb);
        }
    }
    if let Some(ram_str) = ram_override {
        if let Some(gb) = forellm_core::hardware::parse_memory_size(ram_str) {
            specs = specs.with_ram_override(gb);
        }
    }
    if let Some(cores) = cores_override {
