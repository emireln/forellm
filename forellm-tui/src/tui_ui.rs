...
use forellm_core::fit::{FitLevel, ModelFit, SortColumn};
use forellm_core::hardware::is_running_in_wsl;
use forellm_core::providers;
    ...
    let rows: Vec<Row> = app
        ...
        .map(|&idx| {
            ...
            let mode_color = match fit.run_mode {
                forellm_core::fit::RunMode::Gpu => tc.mode_gpu,
                forellm_core::fit::RunMode::MoeOffload => tc.mode_moe,
                forellm_core::fit::RunMode::CpuOffload => tc.mode_offload,
                forellm_core::fit::RunMode::CpuOnly => tc.mode_cpu,
            };
...
fn compare_badges(fit: &ModelFit) -> String {
    ...
    if fit.run_mode == forellm_core::fit::RunMode::MoeOffload {
        tags.push("Offload");
    }
...
fn draw_detail(frame: &mut Frame, app: &App, area: Rect, tc: &ThemeColors) {
        ...
        Line::from(vec![
            ...
            Span::styled(
                ...
                Style::default().fg(match fit.runtime {
                    forellm_core::fit::InferenceRuntime::Mlx => tc.accent,
                    forellm_core::fit::InferenceRuntime::Vllm => tc.accent_secondary,
                    _ => tc.fg,
                }),
            ),
            Span::styled(
    ...
    if fit.model.is_moe {
        ...
        } else if fit.run_mode == forellm_core::fit::RunMode::Gpu {
            lines.push(Line::from(vec![
                Span::styled("  Strategy:    ", Style::default().fg(tc.muted)),
                Span::styled(
...
fn draw_plan(frame: &mut Frame, app: &App, area: Rect, tc: &ThemeColors) {
    ...
    let mut lines = vec![
        ...
        Line::from(vec![
            ...
            Span::styled(
                "Estimate-based using current ForeLLM fit/speed heuristics.",
                Style::default().fg(tc.warning),
            ),
        ]),
        Line::from(""),
