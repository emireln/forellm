...
ForeLLM is a Rust CLI/TUI tool that matches LLM models against local system hardware (RAM, CPU, GPU). It detects system specs, loads a model database from embedded JSON, scores each model's fit, and presents results in an interactive terminal UI or classic table output. An optional desktop GUI (Electron) includes **Agent Fore**, an Ollama-powered chat agent with tools (read_document, execute_python, web_search, run_command with user confirmation), streaming, and file attachments; see `forellm-gui/docs/AGENT_FORE_ARCHITECTURE.md`.
